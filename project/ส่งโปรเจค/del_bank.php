...
@unlink("Product/".$rs['bn_photo']);
$sql_del = mysqli_query($con,"DELETE FROM ".$bank." WHERE bn_id='".$_GET['ID']."'");

	if($sql_del) {
		echo "<script>alert('ลบข้อมูล เสร็จแล้ว')</script>";
		echo "<meta http-equiv='refresh' content='0; url=admin_bank.php?m_page=".$_GET['m_page']."'>";
	} else {
