...
$sql_select = mysqli_query($con,"SELECT * FROM ".$product_type." WHERE type_id='".$_GET['type_id']."'");
...
?>
        <tr>
          <td align="left" valign="top"><div class="title">
              <h2><img src="images/diagram-60.png" width="48" height="48" />
                <?=$type_name?>
              </h2>
          </div>
...
if(!$_GET['type_id']==""){
	$Qtotal = mysqli_query($con,"SELECT * FROM  ".$product." WHERE prd_type='".$_GET['type_id']."'"); //คิวรี่ คำสั่ง
	$total = mysqli_num_rows($Qtotal); // หาจำนวน record 
	$Query = mysqli_query($con,"SELECT * FROM ".$product." WHERE prd_type='".$_GET['type_id']."' ORDER BY prd_id DESC LIMIT $start,$limit"); //คิวรี่คำสั่ง
}else{
                              ...
                              <div id="prd_line_height">
                                <ul id="prd_ul" style="margin-top: 10px;">
								  ...
								  <li> ราคา: <samp  style="color:red;">
								   ...
								   <li style="font-size:12px;"> สต๊อกสินค้า :<samp style="color:red;"><?=$result['prd_stock']?>
                                </samp>ชิ้น</li>
...
?>
		...
		echo "<ul id='ulBangNa' style='text-align: left;'>
				<li>
		";
 		$p_link="type_id=".$_GET['type_id']."";
		echo "<a href='?".$p_link."&start=".$limit*($i-1)."&page=$i' style='border:0px;'><B>หน้าแรก</B></a>";
		echo "</li>";
		echo "<li id='liBangNa'>";
		for($i=1;$i<=$page;$i++){
if($_GET['page']==$i){ //ถ้าตัวแปล page ตรง กับ เลขที่วนได้
echo "<a href='?".$p_link."&start=".$limit*($i-1)."&page=$i' style='border:0px;'><B id='onHoverN''>$i</B></a>"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 1
}else{
echo "<a href='?".$p_link."&start=".$limit*($i-1)."&page=$i' style='border:0px;'>$i</a>"; //ลิ้งค์ แบ่งหน้า เงื่อนไขที่ 2
}
}  
echo "</li>";
echo "<li>";
//echo "หน้าสุดท้าย";
echo "<a href='?".$p_link."&start=".$limit*($i-2)."&page=$i' style='border:0px;'><b>หน้าสุดท้าย</b></a>";
