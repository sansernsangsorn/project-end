	  ...
	  <table width="100%" height="850" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td align="left" valign="top">
                <div class="title">
                    <h2><img src="images/pencil_48.png" width="48" height="48" />แก้ไขสมาชิก</h2>
                </div>
...
?>
				...
				<p>&nbsp;</p>				
				<div class="title">
                    <h2><img src="images/diagram-14.png" width="48" height="48" />ข้อมูลสมาชิก</h2>
                </div>
				<p>&nbsp;</p>				
...
$Search = trim($_POST['txtSearch']); //ตัดซ่องวางของสตริง
	...
	$Qtotal = mysqli_query($con,"SELECT * FROM  ".$member." WHERE mb_status IN('M','N')"); //คิวรี่ คำสั่ง
	$total = mysqli_num_rows($Qtotal); // หาจำนวน record 
	$Query = mysqli_query($con,"SELECT * FROM ".$member." WHERE mb_status IN('M','N') ORDER BY mb_id DESC LIMIT $start,$limit"); //คิวรี่คำสั่ง
			 ...
			 // ถ้ามีรูปสินค้าให้แสดง แต่ถ้าไม่มีให้ แสดงภาพรอรูป
			  if(!$result['mb_photo']==""){ ?>
						...
						<?PHP if($_SESSION['sess_emp_status']=='1'){ ?>
									...
									<a href="del_member.php?m_page=<?=$_GET['m_page']?>&ID=<?=$result['mb_id']?>"  onclick="return confirm('ยืนยัน ลบสมาชิก <?=$result['mb_name']?> ออกจากระบบ')"> ลบ </a>		<? }else{ ?>
...
$birthday =  $year."-".$month."-".$day; // ทำให้เป็นรูปแบบวันเดือนปีที่ใช้งานได้จริง
	...
	echo "<meta http-equiv='refresh' content='0; url=admin_member.php?m_page=".$_POST['m_page']."'>";
	}else{
		$Date = date("Y-m-d");
		$sql_update2 = mysqli_query($con,"UPDATE ".$member." SET "
							." mb_pass ='".$_POST['txtPass']."', 
