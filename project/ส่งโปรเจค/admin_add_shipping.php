			  ...
			  if(!$result['bn_photo']==""){ ?>
                              ...
                              <div id="prd_line_height">
                                <ul id="prd_ul" style="margin-left: 70px;">
                                  ...
                                  <li style="padding-top:5px;"> <a href="edit_bank.php?m_page=<?=$_GET['m_page']?>&amp;ID=<?=$result['bn_id']?>">แก้ไข</a> | <a href="del_bank.php?m_page=<?=$_GET['m_page']?>&amp;ID=<?=$result['bn_id']?>"  onclick="return confirm('ยืนยัน การลบข้อมูลธนาคาร <?=$result['bn_bank']?> ออกจากระบบ')"> ลบ </a> </li>
                                </ul>
                              </div></td>
                          <?php
	if($c == 0) {
