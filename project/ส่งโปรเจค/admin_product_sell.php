	  ...
	  <table width="100%" height="850" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td align="left" valign="top">
                <div class="title">
                    <h2><img src="images/diagram-60.png" width="48" height="48" />ข้อมูลสินค้าขายดี</h2>
                </div>
				<div style="padding:10px; font-size:15px; font-weight:bold; color:#FF0000; border-bottom: 1px solid #ddd;">
				  <form id="form1" name="form1" method="post" action="admin_product.php?page=<?=$_GET['page']?>">
				    <img src="images/Search.png" />ชื่อสินค้า : 
...
$Search = trim($_POST['txtSearch']); //ตัดซ่องวางของสตริง
	...
	$Qtotal = mysqli_query($con,"SELECT * FROM  ".$product." WHERE(prd_name LIKE '%".$Search."%'  AND prd_sell > 0 ) "); //คิวรี่ คำสั่ง
	$total = mysqli_num_rows($Qtotal); // หาจำนวน record 
	$Query = mysqli_query($con,"SELECT * FROM ".$product." WHERE(prd_name LIKE '%".$Search."%' AND prd_sell > 0 ) ORDER BY prd_sell DESC LIMIT $start,$limit"); //คิวรี่คำสั่ง
	...
	<form action="add_stock_product.php?m_page=<?=$_GET['m_page']?>&amp;ID=<?=$result['prd_id']?>" method="post" name="from1<?=$result['prd_id']?>" id="from1" onsubmit="return chk_txt<?=$result['prd_id']?>();">
     ...
     <img src="images/buy_16.png" width="16" height="16" />     <b> สต๊อกสินค้า</b>:<samp style="color:red;">
                                <?=$result['prd_stock']?>
                                          ...
                                          <input name="txt_num"  type="number" min="1" max="1000000" id="txt_num<?=$result['prd_id']?>" style="width: 30px; text-align:center;" />
