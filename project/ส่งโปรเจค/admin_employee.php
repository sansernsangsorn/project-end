            ...
            <tr>
              <td align="left" valign="top"><div class="title">
				...
				<div style="padding:10px; font-size:15px; font-weight:bold; color:#FF0000; border-bottom: 1px solid #ddd;">
				  <form id="form1" name="form1" method="post" action="admin_employee.php?m_page=<?=$_GET['m_page']?>">
				    <img src="images/Search.png" />ชื่อพนักงาน : 
...
$Search = trim($_POST['txtSearch']); //ตัดซ่องวางของสตริง
	...
	$Qtotal = mysqli_query($con,"SELECT * FROM  ".$member." WHERE(mb_name LIKE '%".$Search."%' AND mb_status IN('2'))"); //คิวรี่ คำสั่ง
	$total = mysqli_num_rows($Qtotal); // หาจำนวน record 
	$Query = mysqli_query($con,"SELECT * FROM ".$member." WHERE(mb_name LIKE '%".$Search."%' AND mb_status IN('2')) ORDER BY mb_id DESC LIMIT $start,$limit"); //คิวรี่คำสั่ง
